...
[EIP-5792](https://eips.ethereum.org/EIPS/eip-5792) adds new JSON-RPC methods that enable applications to send multiple calls to a user's wallet. These calls can be enhanced by `capabilities` (such as paymasters), if they are supported by the wallet.
