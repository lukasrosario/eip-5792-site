...
More specifically, this introduces a `wallet_sendCalls` method (with additional methods to check the status of submitted calls and display call information). Apps can specify whether they require atomic execution of calls using the `atomicRequired` field.
...
For wallets, batched calls provide more context on the application's intent, enabling richer and more informative confirmation dialogs, rather than looking at each transaction one by one. Wallets can also indicate their support for atomic execution of batches, allowing apps to make better decisions about how to structure their calls.
...
This EIP is currently in "Review", and the team expect to move to "Final Call" soon. There is already significant support for the EIP across [wallets](/ecosystem/wallets) and [tools](/ecosystem/tools).
