...
[EIP-5792](https://eips.ethereum.org/EIPS/eip-5792) adds new JSON-RPC methods to the wallet interface. These methods will enable applications to send multiple calls to a user's wallet. These calls can be enhanced by `capabilities` (such as paymasters), if they are supported by the wallet.

More specifically, this introduces a `wallet_sendCalls` method, which allows applications to submit multiple onchain calls in a single transaction (with additional methods to check the status of submitted calls and display call information).
...
Review the [EIP](https://eips.ethereum.org/EIPS/eip-5792), join the conversation on [Ethereum Magicians](https://ethereum-magicians.org/t/eip-5792-wallet-function-call-api/11374) and join the [Telegram group](<TODO>).